function apt_create_new_tag(){
	var apt_box_tag_name = jQuery('#apt_box_tag_name').val();
	var apt_box_tag_related_words = jQuery('#apt_box_tag_related_words').val();
	 
	var data = {
		action: 'apt_meta_box_create_new_tag',
	...
	jQuery.ajax ({
		...
		success: function(response) {
			...
			jQuery('#apt_box_tag_name, #apt_box_tag_related_words').val(''); //delete contents of the inputs
...
jQuery(function(){
	//bind apt_create_new_tag() and execute both functions that prevent submitting the form and the other one that adds new tags to DB
	jQuery('#apt_meta_box_create_new_tag_button').click(function(){apt_create_new_tag();});
});
